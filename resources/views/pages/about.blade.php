...
<section class="bg-gradient-warm text-white py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p class="text-xl text-primary-100 max-w-2xl mx-auto">{{ $siteSettings['site_tagline'] ?? 'Donde florece la esperanza, renace una nueva vida.' }}</p>
    </div>
</section>
        ...
        <div class="grid lg:grid-cols-2 gap-16 items-start">
            <div>
                ...
                <div class="prose prose-lg text-gray-600 max-w-none">
                    {!! nl2br(e($siteSettings['about_history'] ?? '')) !!}
                </div>
                <div class="mt-8 flex gap-6">
                    <div class="text-center">
                        <p class="text-4xl font-bold text-primary-600">{{ $siteSettings['founded_year'] ?? '2021' }}</p>
                        <p class="text-sm text-gray-500">Año de fundación</p>
                    </div>
                    <div class="w-px bg-gray-200"></div>
                    <div class="text-center">
                        <p class="text-4xl font-bold text-secondary-600">{{ $siteSettings['legal_year'] ?? '2026' }}</p>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="card text-center p-8">
                ...
                <h3 class="font-heading font-bold text-gray-800 text-xl mb-1">{{ $member->name }}</h3>
                <p class="text-primary-600 font-semibold mb-3">{{ $member->role }}</p>
                @if($member->bio)
                <p class="text-gray-500 text-sm leading-relaxed">{{ $member->bio }}</p>
                @endif
                @if($member->email || $member->linkedin || $member->facebook)
                <div class="flex justify-center gap-3 mt-4">
                    @if($member->email)
                    <a href="mailto:{{ $member->email }}" class="text-gray-400 hover:text-primary-600 transition-colors">
