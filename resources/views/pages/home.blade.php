    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 grid lg:grid-cols-2 gap-12 items-center">
        ...
        <div class="text-white" x-data x-intersect="$el.classList.add('animate-fade-in')">
            ...
            <h1 class="text-4xl md:text-5xl lg:text-6xl font-heading font-extrabold leading-tight mb-6">
                {{ $siteSettings['hero_title'] ?? 'Juntos construimos caminos de esperanza' }}
            </h1>
            <p class="text-lg md:text-xl text-primary-100 leading-relaxed mb-10 max-w-xl">
                {{ $siteSettings['hero_subtitle'] ?? 'Apoyamos a personas y familias en situación de vulnerabilidad mediante acciones integrales que promueven el bienestar, la inclusión y el desarrollo.' }}
            </p>
            <div class="flex flex-wrap gap-4">
                <a href="{{ route('programs.index') }}" class="btn-secondary">
                    {{ $siteSettings['hero_cta_text'] ?? 'Conoce nuestro trabajo' }}
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                </a>
                <a href="{{ route('donations') }}" class="btn-outline">
                    {{ $siteSettings['hero_cta2_text'] ?? 'Quiero donar' }} ❤️
                </a>
            </div>
        </div>
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-3xl p-8 border border-white/20 max-w-sm w-full">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="text-center p-4 bg-white/10 rounded-2xl">
                        <p class="text-4xl font-bold text-secondary-300" data-counter>{{ $siteSettings['stat_families'] ?? '500+' }}</p>
                        <p class="text-white/70 text-xs mt-1">Familias apoyadas</p>
                    </div>
                    <div class="text-center p-4 bg-white/10 rounded-2xl">
                        <p class="text-4xl font-bold text-accent-300" data-counter>{{ $siteSettings['stat_volunteers'] ?? '50+' }}</p>
                        <p class="text-white/70 text-xs mt-1">Voluntarios</p>
                    </div>
                    <div class="text-center p-4 bg-white/10 rounded-2xl">
                        <p class="text-4xl font-bold text-lilac-300" data-counter>{{ $siteSettings['stat_programs'] ?? '10+' }}</p>
                        <p class="text-white/70 text-xs mt-1">Programas activos</p>
                    </div>
                    <div class="text-center p-4 bg-white/10 rounded-2xl">
                        <p class="text-4xl font-bold text-primary-200" data-counter>{{ $siteSettings['stat_years'] ?? '5' }}</p>
        ...
        <div class="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div class="relative">
                ...
                <div class="absolute -bottom-5 -right-5 bg-secondary-500 text-white rounded-2xl px-5 py-3 shadow-lg font-heading font-bold text-center">
                    <p class="text-3xl font-extrabold">{{ ($siteSettings['legal_year'] ?? '2026') - ($siteSettings['founded_year'] ?? '2021') }}+</p>
            ...
            <div>
                ...
                <p class="text-gray-600 leading-relaxed mb-6">
                    {{ Str::limit($siteSettings['about_history'] ?? 'Sinergia de Amor y Esperanza A.C. nace en abril del 2021, en medio de una etapa difícil para muchas familias debido a la crisis ocasionada por la pandemia.', 300) }}
                </p>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid md:grid-cols-2 gap-8">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-md border-l-4 border-primary-500 hover:shadow-xl transition-shadow">
                ...
                <p class="text-gray-600 leading-relaxed">{{ $siteSettings['mission'] ?? 'Brindamos apoyo integral a grupos vulnerables en México, así como en el ámbito internacional, promoviendo su bienestar, inclusión y calidad de vida.' }}</p>
            </div>
            {{-- Visión --}}
            <div class="bg-white rounded-3xl p-8 shadow-md border-l-4 border-secondary-500 hover:shadow-xl transition-shadow">
                ...
                <p class="text-gray-600 leading-relaxed">{{ $siteSettings['vision'] ?? 'Para el año 2028, consolidarnos como una organización con presencia nacional e internacional, reconocida por su capacidad de brindar apoyo integral.' }}</p>
            </div>
        </div>
    </div>
</section>
...
<section class="section-padding bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            @endphp
            <a href="{{ route('programs.show', $program->slug) }}" class="card-program {{ $c['bg'] }} group">
                <div class="w-12 h-12 {{ $c['icon'] }} rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
                    </svg>
                </div>
                <h3 class="font-heading font-bold {{ $c['title'] }} mb-2 text-lg">{{ $program->title }}</h3>
                <p class="text-gray-600 text-sm leading-relaxed">{{ $program->short_description }}</p>
                <div class="flex items-center gap-1 mt-4 {{ $c['title'] }} text-sm font-semibold">
                    Ver más
        ...
        <div class="text-center mt-10">
            <a href="{{ route('programs.index') }}" class="btn-primary">Ver todos los programas</a>
        </div>
    </div>
</section>
@endif
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="card text-center p-8 group">
                ...
                <h3 class="font-heading font-bold text-gray-800 text-lg mb-1">{{ $member->name }}</h3>
                <p class="text-primary-600 font-semibold text-sm mb-3">{{ $member->role }}</p>
                @if($member->bio)
...
<section class="section-padding bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-14 gap-4">
            ...
            <a href="{{ route('blog.index') }}" class="btn-primary self-start sm:self-auto">Ver todo el blog</a>
        </div>
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <article class="card group">
                ...
                <div class="p-6">
                    ...
                    @if($post->excerpt)
                    <p class="text-gray-500 text-sm leading-relaxed mb-4">{{ Str::limit($post->excerpt, 120) }}</p>
                    @endif
                    <div class="flex items-center justify-between text-xs text-gray-400">
                        <span>{{ $post->published_at?->format('d M, Y') }}</span>
                        <a href="{{ route('blog.show', $post->slug) }}" class="text-primary-600 font-semibold hover:underline">Leer más →</a>
                    </div>
                </div>
            </article>
            @endforeach
...
<section class="section-padding bg-gradient-to-r from-accent-500 to-orange-600 text-white">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div class="flex flex-wrap justify-center gap-4">
            <a href="{{ route('donations') }}" class="bg-white text-accent-600 font-bold px-8 py-4 rounded-full hover:shadow-xl hover:-translate-y-0.5 transition-all">
                Quiero Donar Ahora
            </a>
            <a href="{{ route('volunteer.index') }}" class="border-2 border-white text-white font-bold px-8 py-4 rounded-full hover:bg-white/10 transition-all">
                Ser Voluntario
            </a>
        </div>
    </div>
