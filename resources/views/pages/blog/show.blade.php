@extends('layouts.app')

@section('title', $post->title)
@section('meta_description', $post->meta_description ?? $post->excerpt)

@section('content')

<section class="bg-gradient-warm text-white py-16">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <a href="{{ route('blog.index') }}" class="inline-flex items-center gap-2 text-primary-200 hover:text-white mb-6 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            Volver al Blog
        </a>
        @if($post->category)
        <span class="badge bg-white/20 text-white mb-4">{{ $post->category }}</span>
        @endif
        <h1 class="text-3xl md:text-4xl font-heading font-extrabold leading-tight mb-4">{{ $post->title }}</h1>
        <div class="flex items-center gap-4 text-primary-200 text-sm">
            <span>{{ $post->author_name }}</span>
            <span>•</span>
            <span>{{ $post->published_at?->format('d \d\e F, Y') }}</span>
            <span>•</span>
            <span>{{ $post->reading_time }} min de lectura</span>
        </div>
    </div>
</section>

<section class="section-padding bg-warm-50">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">

        @if($post->image)
        <img src="{{ asset('images/' . $post->image) }}" alt="{{ $post->title }}" class="w-full h-72 md:h-96 object-cover rounded-3xl shadow-lg mb-10">
        @endif

        <div class="bg-white rounded-3xl shadow-md p-8 md:p-12">
            <div class="prose prose-lg max-w-none text-gray-700">
                {!! $post->content !!}
            </div>

            @if($post->tags && count($post->tags))
            <div class="flex flex-wrap gap-2 mt-8 pt-6 border-t border-gray-100">
                @foreach($post->tags as $tag)
                <span class="badge badge-primary">{{ $tag }}</span>
                @endforeach
            </div>
            @endif

            {{-- Compartir --}}
            <div class="mt-8 pt-6 border-t border-gray-100 flex items-center gap-4">
                <span class="text-gray-500 text-sm font-medium">Compartir:</span>
                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" target="_blank" class="text-blue-600 hover:text-blue-800 transition-colors">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
                <a href="https://wa.me/?text={{ urlencode($post->title . ' ' . url()->current()) }}" target="_blank" class="text-green-600 hover:text-green-800 transition-colors">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                </a>
            </div>
        </div>

        {{-- Artículos relacionados --}}
        @if($related->count())
        <div class="mt-14">
            <h2 class="text-2xl font-heading font-bold text-gray-800 mb-6">Artículos Relacionados</h2>
            <div class="grid sm:grid-cols-3 gap-6">
                @foreach($related as $article)
                <article class="card group">
                    <div class="h-36 bg-gradient-warm overflow-hidden">
                        @if($article->image)
                            <img src="{{ asset('images/' . $article->image) }}" alt="{{ $article->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                        @else
                            <div class="w-full h-full flex items-center justify-center text-3xl text-white/80">📰</div>
                        @endif
                    </div>
                    <div class="p-4">
                        <h3 class="font-heading font-bold text-gray-800 group-hover:text-primary-600 transition-colors mb-2">
                            <a href="{{ route('blog.show', $article->slug) }}">{{ Str::limit($article->title, 60) }}</a>
                        </h3>
                        <span class="text-xs text-gray-400">{{ $article->published_at?->format('d M, Y') }}</span>
                    </div>
                </article>
                @endforeach
            </div>
        </div>
        @endif
    </div>
</section>

@endsection
